...
function DormSecretariesInner() {
  ...
  useEffect(() => {
    ...
    fetch(`/api/reports/dorm-secretaries?${params}`)
      .then((r) => r.json())
        ...
        <table className="w-full text-sm">
          ...
          <tbody>
            {loading ? (
              ...
              <tr><td colSpan={6}>
                <EmptyState icon="📋" title="No secretaries found" description="Try adjusting your filters." />
              </td></tr>
            ) : (
              rows.map((r) => (
        ...
        {loading ? <LoadingCards count={4} /> : rows.length === 0 ? (
          ...
          <div key={r.id} className="border border-primary/15 rounded-sm p-4">
            <div className="flex items-center justify-between mb-1">
              ...
              <span className="text-xs bg-primary/10 text-primary px-2 py-0.5 rounded-sm">{formatRole(r.role)}</span>
            </div>
...
export default function DormSecretariesPage() {
  return <Suspense><DormSecretariesInner /></Suspense>;
}
